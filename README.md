<!--- Three different levels of headers with bold text formatting for  --->

# **Stats 220**

### This is my repo for **STATS 220**.

## A little about me:

<!--- unordered list --->

- I am undertaking a graduate diploma in stats
- I am taking STATS 220 because I'd like an overview of data technologies that can help with the analysis of data
- I am interested in learning about how stats can be used at work.

<!--- links to other websites
One of my favourite sites is [Chat GPT](https://chatgpt.com)

<!--- ordered list with links to different animated GIFs --->

A few memes that capture:

1. How I *sometimes* feel about my university studies:

![](https://media1.tenor.com/m/sZNE7ibNb_sAAAAd/goalie-catch.gif)

2. How I *mostly* feel about my studies:

![](https://media1.tenor.com/m/xN_7QhSmr84AAAAd/homer-simpsons.gif)

3. How I felt coming back to uni after *decades*:

![](https://media1.tenor.com/m/GhzETztJlmUAAAAd/serious-lee-seriously.gif)

